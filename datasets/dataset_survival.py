                ...
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        if not os.path.exists(wsi_path):
                            if len(slide_ids) == 1:
                                path_features.append(torch.ones(1))
                                path_coords.append(torch.ones(1))
                        else:
                            h5_file = h5py.File(wsi_path, 'r')
                            wsi_bag = torch.tensor(np.array(h5_file['features']))
                            if temp is None:
                                coords = torch.tensor(np.array(h5_file['coords']))/512
                                temp = coords.max(dim=0, keepdim=True)[0] + 10
                            else:
                                coords = torch.tensor(np.array(h5_file['coords']))/512 + temp
                                temp = coords.max(dim=0, keepdim=True)[0] + 10
                    ...
                    path_coords = torch.cat(path_coords, dim=0)
                    return (path_features, path_coords, label, event_time, c, wsi_path)

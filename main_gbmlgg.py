...
# Pathing Parameters
parser.add_argument('--data_root_dir',   type=str, default='/data/alven/wenhuasdrive/MNHS-SPHPM-CARE/GLOBALENVIRONHealth/CMIP6/Generated_by_zk/TCGA/Glioma', help='Data directory to WSI features (extracted via CLAM')
parser.add_argument('--split_dir',       type=str, default='tcga_gbmlgg', help='Which cancer type within ')

parser.add_argument('--affinity_threshold', type=float, default=0.85)

parser.add_argument('--num_instances_maximum', type=int, default=30000)
...
def main():
    ...
    cindex_small_loss_list, cindex_final_list, auc_small_loss_list, auc_final_list = [], [], [], []
    ...
    for i in folds:
        ...
        cindex_small_loss, cindex_final, auc_small_loss, auc_final = train(datasets, i, args)

        cindex_small_loss_list.append(cindex_small_loss)
        cindex_final_list.append(cindex_final)
        auc_small_loss_list.append(auc_small_loss)
        auc_final_list.append(auc_final)

    cindex_small_loss_list = np.array(cindex_small_loss_list)
    cindex_final_list = np.array(cindex_final_list)
    auc_small_loss_list = np.array(auc_small_loss_list)
    auc_final_list = np.array(auc_final_list)

    print('\n')
    print('c-index small loss {}'.format(cindex_small_loss_list))
    print('Average c-index small loss {} + {} '.format(cindex_small_loss_list.mean(), cindex_small_loss_list.std() ))
    print('c-index final {}'.format(cindex_final_list))
    print('Average c-index final {} + {}'.format(cindex_final_list.mean(), cindex_final_list.std()))
    print('\n')
    print('auc small loss {}'.format(auc_small_loss_list))
    print('Average auc small loss {} + {} '.format(auc_small_loss_list.mean(), auc_small_loss_list.std() ))
    print('auc final {}'.format(auc_final_list))
    print('Average auc final {} + {}'.format(auc_final_list.mean(), auc_final_list.std()))
...
if __name__ == "__main__":
    ...
    sys.stdout = Logger('./log/' + str(files) + '_' + 'filtering_small_patches_test' + str(args.filtering_small_patches_test) + '.txt')
